 ...
 *
 */
class WxPayApi extends AWS_MODEL
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetAppid($WxPayConfig->wxpay_openid);//公众账号ID
		$inputObj->SetMch_id($WxPayConfig->wxpay_mchid);//商户号
     ...
     *
     * 企业付款给用户
     * partner_trade_no、openid、re_user_name、amount为必填参数
     * appid、mchid、spbill_create_ip、nonce_str不需要填入
     * @param WxPayToUser $inputObj
     * @param int $timeOut
     * @throws WxPayException
     * @return 成功时返回，其他抛异常
     */
    public static function toUser($inputObj, $timeOut = 6)
    {
        $WxPayConfig = new WxPayConfig();
        $url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/promotion/transfers";
        //检测必填参数
        if(!$inputObj->IsPartner_trade_noSet()) {
            throw new WxPayException("商户订单号必填");
        }else if(!$inputObj->IsOpenidSet()){
            throw new WxPayException("用户Openid必填");
        }else if(!$inputObj->IsAmountSet()){
            throw new WxPayException("付款金额提必填");
        }else if(!$inputObj->IsDescSet()){
            throw new WxPayException("信息");
        }
        ...
        $inputObj->SetSpbill_create_ip('115.28.203.86');
        $inputObj->SetSign();//签名
        $xml = $inputObj->ToXml();
        $startTimeStamp = self::getMillisecond();//请求开始时间
        $response = self::postXmlCurl($xml, $url, true, $timeOut);
