...
class shang_aliwap_class extends AWS_MODEL
{
	private $alipay_partner;
	private $alipay_seller_email;
	private $alipay_key;
	private $alipay_transport = 'http';
	private $alipay_input_charset = 'utf-8';
	private $alipay_private_key;
	private $alipay_ali_public_key;
	private $alipay_sign_type = 'MD5';
	
	public function __construct(){
		$this->alipay_partner = get_setting('alipay_partner');
		$this->alipay_seller_email = get_setting('alipay_seller_email');
		$this->alipay_key = get_setting('alipay_key');
    ...
    public function buildRequestMysign($para_sort)
    {
        ...
        switch ($this->alipay_sign_type) {
            case "MD5":
                $mysign = $this->md5Sign($prestr, $this->alipay_key);

                break;

            case "RSA":
            case "0001":
                $mysign = $this->rsaSign($prestr, $this->alipay_private_key);
    ...
    public function buildRequestPara($para_temp)
    {
        ...
        if ($para_sort['service'] != 'alipay.wap.trade.create.direct' && $para_sort['service'] != 'alipay.wap.auth.authAndExecute') {
            $para_sort['sign_type'] = $this->alipay_sign_type;
        }
    ...
    public function buildRequestHttp($para_temp)
    {
        ...
        // 远程获取数据
        $sResult = $this->getHttpResponsePOST($this->alipay_gateway_new, $this->cacert, $request_data, $this->alipay_input_charset);
    ...
    public function buildRequestHttpInFile($para_temp, $file_para_name, $file_name)
    {
        ...
        // 远程获取数据
        $sResult = $this->getHttpResponsePOST($this->alipay_gateway_new, $this->cacert, $para, $this->alipay_input_charset);
    ...
    function buildRequestForm($para_temp, $method, $button_name)
    {
        ...
        $sHtml = "<form id='alipaysubmit' name='alipaysubmit' action='".$this->alipay_gateway_new."_input_charset=".$this->alipay_input_charset."' method='".$method."'>";
    ...
    function buildRequestForm2($para_temp, $method, $button_name)
    {
        //待请求参数数组
        $para = $this->buildRequestPara($para_temp);
    ...
    public function parseResponse($str_text)
    {
        ...
        if ($para_text['res_data']) {
            // 解析加密部分字符串
            if ($this->alipay_sign_type == '0001') {
                $para_text['res_data'] = $this->rsaDecrypt($para_text['res_data'], $this->alipay_private_key);
            }
    ...
    {
        $url = $this->alipay_gateway_new . 'service=query_timestamp&partner=' . $this->alipay_partner . '&_input_charset=' . $this->alipay_input_charset;
        $encrypt_key = '';
    ...
    function verifyNotify()
    {
        ...
        if ($this->alipay_sign_type == '0001') {
            $decrypt_post_para['notify_data'] = $this->rsaDecrypt($decrypt_post_para['notify_data'], $this->alipay_private_key);
        }
    ...
    {
        return $this->rsaDecrypt($prestr, $this->alipay_private_key);
    }
    ...
    function getSignVeryfy($para_temp, $sign, $isSort)
    {
        ...
        switch ($this->alipay_sign_type) {
            case 'MD5':
                $isSgin = $this->md5Verify($prestr, $sign, $this->alipay_key);

                break;

            case 'RSA':
            case '0001':
                $isSgin = $this->rsaVerify($prestr, $this->alipay_ali_public_key, $sign);
